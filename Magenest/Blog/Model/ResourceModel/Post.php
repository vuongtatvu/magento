...
use Magenest\Blog\Model\TagFactory as TagModelFactory;
...
class Post extends AbstractEntity
{
    ...
    {
        if (empty($this->_type)) {
            $this->setType(\Magenest\Blog\Model\Post::ENTITY);
        }
    ...
    public function getCategoryIds($post)
    {
        ...
        $select = $connection->select()->from(
            $this->getTable('magenest_blog_category_post'),
            'category_id'
        )->where(
            'post_id = ?',
            (int)$post->getId()
    ...
    public function getStoreIds($post)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('magenest_blog_store_post'),
            'store_id'
        )->where(
            'post_id = ?',
            (int)$post->getId()
    ...
    public function getTagIds($post)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('magenest_blog_tag_post'),
            'tag_id'
        )->where(
            'post_id = ?',
            (int)$post->getId()
    ...
    public function getProductIds($post)
    {
        $connection = $this->getConnection();
        $select = $connection->select()->from(
            $this->getTable('magenest_blog_post_product'),
            'product_id'
        )->where(
            'post_id = ?',
            (int)$post->getId()
    ...
    protected function _beforeSave(DataObject $post)
    {
        ...
        if (!$post->hasData('type')) {
            $post->setData('type', \Magenest\Blog\Model\Post::TYPE_POST);
        }
    ...
    {
        $table = $this->getTable('magenest_blog_post_product');
    ...
    {
        $table = $this->getTable('magenest_blog_category_post');
        /**
    ...
    {
        $table = $this->getTable('magenest_blog_store_post');
        /**
    ...
    {
        $table = $this->getTable('magenest_blog_tag_post');
